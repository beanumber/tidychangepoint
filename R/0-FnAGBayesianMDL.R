...
revisor_param <- function(param,
  ...
  my_data <- 123
  ejemplo_param <- list(nombre_datos = c("ciclones","O3","O3_U61","PM10","PM10_U100")[5],
                        frecuencia_datos = 7, # e.g., 7 equivale a semanal
                        n_datos = c("TODOS",403,1003)[1], # tomamos todos o un subconjuto de datos
                        diarios0_rebases1 = 0,
                        valor_de_rebase = 100,
  ...
  if(!all(param$mat_low_upp[,2]-param$mat_low_upp[,1]>0)) {
    cat("La matriz param$mat_low_upp es incorrecta; algunos valores low son",
  ...
  if(error1_bien0 == 1) cat("EXISTEN ERRORES EN param.\n") else
...
#' Generador de texto para evaluar una lista param
#'
#' @param renglon_parametros renglón extraido del excel 
#'
#' @return
#' @export
#'
...
genera_texto_eval_param <- function(renglon_parametros) {
  # Generamos una matriz de cuatro columnas con los textos requeridos
  matriz_textos <- matrix(c(names(renglon_parametros[-1:-2]),
                            rep("=",length(renglon_parametros[-1:-2])),
                            as.character(renglon_parametros[-1:-2]),
                            rep(",",length(renglon_parametros[-1:-2])-1),""),nrow = 4,byrow = T)
  # Pegamos los valores de esta matriz
  texto_medio <- paste(matriz_textos,collapse = "")
  # Agregamos lo necesario para completar el texto
  texto_list_param <- paste0("param <- list(",texto_medio,")")
  return(texto_list_param)
}

#' Title
#'
#' @param archivo_excel nombre del archivo excel; eg "Parametros pruebas V01.xlsx"
#' @param hoja_excel nombre de la hoja que se ejecuta; eg "V01"
#'
#' @return
#' @export
#'
#' @examples
Ejecuta_multiples_pruebas_AG <-  function(archivo_excel,hoja_excel) {
  # Extraemos los datos del excel
  matriz_parametros <- read.xlsx(xlsxFile = archivo_excel, sheet = hoja_excel, 
                                 skipEmptyRows = FALSE,colNames = T)
  
  # Evaluando cada renglon de matriz_parametros
  for(i in 1:nrow(matriz_parametros)) {
    # Extraemos el renglón i-ésimo
    renglon_parametros <- matriz_parametros[i,]
    cat(" --- ",i," de ",nrow(matriz_parametros),": evaluando No. prueba ",
        as.character(renglon_parametros[2])," con fecha ",
        as.character(renglon_parametros[1])," --- \n")
    # Obtenemos el texto del renglon
    texto_list_param <- genera_texto_eval_param(renglon_parametros)
    # Evaluamos el texto para generar la lista param
    eval(parse(text=texto_list_param))
    # Revisamos que los valores de param sean válidos
    revisor_param(param)
    # Ejecutamos el algoritmo genético
    AG_BMDL_r_paso(param)
  }
}
...
#' Ejecución de multiples pruebas de AG extrayendo de un archivo excel
#'
#' @param archivo_excel nombre del archivo excel; eg "Parametros pruebas V01.xlsx"
#' @param hoja_excel nombre de la hoja que se ejecuta; eg "V01"
#'
...
AG_BMDL_1_paso <- function(x,mat_cp,param){
  ...
  (vec_BMDL_k_cp <- Bayesaian_MDL_k_cp(mat_cp,x,param$rf_type,
...
AG_BMDL_r_paso <- function(param){
  ...
  nombre_pdf <- paste0(param$nombre_carpeta_pdf, "/Fig_4AGBMDL_",param$nombre_datos,"_rf_",
...
Bloq_LogPrio_NHPP <- function(vec_dist_a_priori,theta,mat_phi) {
  ...
  }else print("No se tiene registrada esa vec_dist_a_priori; Bloq_LogPrio_NHPP")
...
Bloq_LogVero_NHPP <- function(vec_d_i,tau1,tau2,rf_type,theta){
  if(rf_type=="W") return( (tau1^theta[1]-tau2^theta[1]) / theta[2]^theta[1] + 
                             ...
                             (theta[1]-1)*sum(log(vec_d_i)) )#Expresion pg 15 entre las expresiones 18 y 19
  if(rf_type=="EW") {
    difN <- length(vec_d_i)
    alpha <- theta[1]
    beta <- theta[2]
...
D_Bloq_LogPrio_NHPP <- function(vec_dist_a_priori,theta,mat_phi) {
  ...
  } else print("No se tiene registrada esa vec_dist_a_priori; D_Bloq_LogPrio_NHPP")
...
D_Bloq_LogVero_NHPP <- function(vec_d_i,tau1,tau2,rf_type,theta){
  ...
  if(rf_type %in% c("W","GO","MO")) return(c(p1,p2)) else return(c(p1,p2,p3))
}
...
extrae_mat_MAP <- function(cp,x,rf_type,initial_val_optim,mat_low_upp,vec_dist_a_priori,mat_phi,ajuste_bloque) {
  ...
  for(i in 1:n_mle){
    ...
    aux_map <- MAP_NHPP(initial_val_optim,mat_low_upp,lista_insumos_bloque$lista_dias_regimen[[i]],
...
fun_1_genera_texto_dist <- function(dist,parametros_dist){
  ...
  } else for(i in 1:10) print("no se tiene esta distribución; fun_1_genera_texto_dist")
  return(texto)
}
...
grafica_escalonada <- function(res, col_segmets, vec_xlim=NULL,vec_ylim=NULL) {
  ...
  # Graficando
  plot(0,ylim=vec_ylim,xlim=vec_xlim,type="n",
...
genera_insumos_bloque_sin_theta <- function(cp,x){
  ...
  (lista_dias_regimen <- apply(mat_intervalos_cp,1, function(y){x[x>y[1] & x<=y[2]]}))
...
graficas_BMDL <- function(lista_AG,param) {
  ...
  mat_MAP <- extrae_mat_MAP(cromo,lista_AG$x,param$rf_type,param$initial_val_optim,
...
junta_1_puntos_cambio <- function(padres,mat_cp) {
  ...
  if(length(hijo)<=ncol(mat_cp)-1){
    ...
    hijo <- c(ncol(mat_cp)-3,1,sort(sample(hijo[c(-1,-length(hijo))],
...
#'
#' @examples
#' t <- c(1.4,2.8); theta <- c(1.2,2.1,3.2)
#' rf_type="EW"; mean_fn(t,rf_type,theta)
#' rf_type="GGO"; mean_fn(t,rf_type,theta)
#' rf_type="MO"; mean_fn(t,rf_type,theta)
#' rf_type="GO"; mean_fn(t,rf_type,theta)
#' 
mean_fn <- function(t,rf_type,theta) {
  if(rf_type=="W") return((t/theta[2])^theta[1])
  if(rf_type=="EW") return(-log(1-(1-exp(-(t/theta[3])^theta[1]))^theta[2]))
  if(rf_type=="GGO") return(theta[1]*(1-exp(-theta[2]*t^theta[3])))
  if(rf_type=="MO") return(theta[2]*log(1+t/theta[1]))
  if(rf_type=="GO") return(theta[1]*(1-exp(-theta[2]*t)))
  else print("no se tiene esa rf_type; mean_fn")
}
...
muta_1_cp_BMDL <- function(cp,x,param) {
  ...
  if(cp[1] <= param$minimo_numero_de_cp) return(sim_1_cp_BMDL(x,param))
  
  (cp_posibles_muta <- cp[3:(cp[1]+2)])
  # Indices mutados
  i_mutados <- sort(unique(sapply(match(cp_posibles_muta,x) + sample(param$mutaciones, size=cp[1], prob=param$probs_muta, replace=T),
                                  function(yy){min(max(yy,param$dist_extremos),length(x)-param$dist_extremos)})))
  # Perturbamos los tau
  ...
  if(length(cp_posibles_muta)< param$max_num_cp-3){
    ...
    if(length(cp_posibles_muta)< param$max_num_cp+cuantos_nuevos && cuantos_nuevos>0){
      cp_posibles_muta <- sort(unique(c(cp_posibles_muta,
...
probs_vec_MDL <- function(vec_MDL,probs_rank0_MDL1) {
  ...
  if(probs_rank0_MDL1==0) return(rank(-vec_MDL))
  if(probs_rank0_MDL1==1) return(-vec_MDL)
}
...
selec_k_pares_de_padres <- function(vec_probs) {
  ...
  if(length(indices_mal)>0) for (i in indices_mal) {
    mamas[i] <- sample((1:k)[-papas[i]],size = 1,
...
#' t <- c(1.4,2.8); theta <- c(1.2,2.1,3.2)
#'
#' rf_type="EW"; rate_fn(t,rf_type,theta)
#' rf_type="GGO"; rate_fn(t,rf_type,theta)
#' rf_type="MO"; rate_fn(t,rf_type,theta)
#' rf_type="GO"; rate_fn(t,rf_type,theta)
#'
#' 
rate_fn <- function(t,rf_type,theta) {
  # W -> theta = c(alpha, sigma)
  if(rf_type=="W") return((theta[1]/theta[2])*(t/theta[2])^(theta[1]-1))
  # EW -> theta = c(alpha,beta,sigma)
  if(rf_type=="EW") return(theta[1]*theta[2]*(1-(exp(-t/theta[3]))^theta[1])^(theta[2]-1)*
                             (exp(-t/theta[3]))^theta[1]*
                             (t/theta[3])^(theta[1]-1)/
                             (theta[3]*(1-(1-exp(-(t/theta[3])^theta[1]))^theta[2])))
  # GGO -> theta = c(alpha,beta,gamma)
  if(rf_type=="GGO") return(theta[1]*theta[2]*theta[3]*t^(theta[3]-1)*exp(-theta[2]*t^theta[3]))
  # MO -> theta = c(alpha,beta)
  if(rf_type=="MO") return(theta[2]/(t+theta[1]))
  # GO -> theta = c(alpha,beta)
  if(rf_type=="GO") return(theta[1]*theta[2]*exp(-theta[2]*t))
  else print("no se tiene esa rf_type; rate_fn")
}
