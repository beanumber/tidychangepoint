MaMBOltzmann <- function(mat_cp, Fitness, Mutation, Temperature){
  ...
  FitnessMatrix <- data.frame(Index = rank(-Fitness),
  ...
  while (dim(Offspring)[1] < (k+1)) {
    ...
    if(FatherCandidate1 == 1){
      ...
      FatherCandidate2 <- sample(c(FatherCandidate1-1,
                                   FatherCandidate1+1), 1)
      
    } else{
      
      FatherCandidate2 <- sample(c(FatherCandidate1-2,
                                   FatherCandidate1-1,
                                   FatherCandidate1+1,
                                   FatherCandidate1+2), 1)
    }
    ...
    if(MotherCandidate1 == 1){
      ...
      MotherCandidate2 <- sample(c(MotherCandidate1-1,
                                   MotherCandidate1+1), 1)
      
    } else{
      
      MotherCandidate2 <- sample(c(MotherCandidate1-2,
                                   MotherCandidate1-1,
                                   MotherCandidate1+1,
                                   MotherCandidate1+2), 1)
    }
    ...
    if(FatherIndex != MotherIndex){
      ...
      Child <- c(mat_cp[FatherIndex, 3:(mat_cp[FatherIndex, 1]+2)],
